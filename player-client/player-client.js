...
3. Make your decision and explain your reasoning in 1-2 entertaining sentences with personality
4. Be strategic but also entertaining for spectators
...
{"action": "fold", "commentary": "Seven-deuce offsuit? Not even with my worst enemy's chips. I'm out."}
{"action": "call", "commentary": "Drawing to a flush here - pot odds say it's worth a shot!"}`;

  try {
    // Create a temporary file with the prompt
    const tmpDir = os.tmpdir();
    const tmpFile = path.join(tmpDir, `poker-prompt-${Date.now()}.txt`);
    fs.writeFileSync(tmpFile, prompt);

    // Call claude CLI with the prompt from stdin
    const { stdout, stderr } = await execAsync(`cat "${tmpFile}" | claude`);
...
async function main() {
  console.log(`\n🃏 Claude Poker Player Client Starting...`);
